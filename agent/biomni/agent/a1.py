  ...
  - Bash scripts and commands: Use the #!BASH marker in your execute block for both simple commands and complex shell scripts with variables, loops, conditionals, etc.
    ...
    def configure(self, self_critic=False, test_time_scale_round=0):
        ...
        def execute(state: AgentState) -> AgentState:
            ...
            execute_matches = re.findall(r"<execute>(.*?)</execute>", last_message, re.DOTALL)
            if execute_matches:
                ...
                for code in execute_matches:
                    ...
                    observations.append(f"<observation>{result}</observation>")

                combined_observation = "\n".join(observations).strip()
                state["messages"].append(AIMessage(content=combined_observation))
    ...
    def go(self, prompt, thread_id: int | None = None):
    ...
    def go_stream(self, prompt, thread_id: int | None = None, chat_history: list | None = None) -> Generator[dict, None, None]:
        ...
        for s in self.app.stream(inputs, stream_mode="values", config=config):
            message = s["messages"][-1]
            out = pretty_print(message, printout=False)  # 🔴 不打印到 stdout
            self.log.append(out)
            final_state = s  # Store the latest state
